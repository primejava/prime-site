package org.primejava.cms.lucene;
...
public class GetSearchResult {
	 ...
	 * @throws Exception
	 */
	public List<DocumentEntity> getResult(String queryStr, String uploadFolder,int currentPageNum,int topNum)
			throws Exception {
		//初始化列表
		String indexDir="/upload/index";
		...
		for (int i = (currentPageNum - 1) * eachePageNum; i < totalNumber; i++) {
			...
			TokenStream tokenStream = new IKAnalyzer(false).tokenStream("contents", new StringReader(doc.get("contents")));
			docEntity.setContents(highlighter.getBestFragment(tokenStream, doc.get("contents")));
			fileName = doc.get("fileName");
			tokenStream = new IKAnalyzer().tokenStream("fileName",new StringReader(fileName));
			...
			} else if ("doc".equalsIgnoreCase(type1)||"docx".equalsIgnoreCase(type1)) {
				//是要获取绝对路径吧
				fileName=fileName+uploadFolder+type1;
