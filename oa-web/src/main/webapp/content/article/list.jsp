...
<body>
	...
	<%@include file="/menu/article.jsp"%>
		...
		<div style="margin-bottom: 20px;">
			<div class="pull-left btn-group" role="group">
				...
				<button class="btn btn-default"
					onclick="location.href='subscribe.do'">订阅</button>
				<button id="test" onclick="test()" class="btn btn-default">ajax测试</button>
		...
		<!-- 工具条结束-->
			...
			<form id="groupGridForm" name="groupGridForm" method='post'
				action="deletegroups.do" class="m-form-blank">
				<table id="groupGrid" class="table table-hover">
					<thead>
						<tr>
							...
							<th class="sorting" name="author">作者</th>
							<th>操作</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${page.result}" var="item">
							<tr>
								...
								<td>${item.title}</td>
								<td>${item.author}</td>
								<td><a href="input.do?id=${item.id}" class="a-update">编辑</a><a href="preview.do?id=${item.id}" class="a-update">预览</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
